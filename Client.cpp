...
map<int, vector<string>>listMessageGroup;
int group = -1;
int codescreen = 0;
string listmessage;
...
// send request to show JoinedGroupList
...
void handleChatGroupMenu() {
	...
	case 3:
		...
		curScreen = CHAT_GROUP_BOX_SCREEN;
		navigateScreen(CHAT_GROUP_BOX_SCREEN);
...
void handleResponse(string msg) {
	...
	// LISTGROUP
		...
		if (joinedGroupList.size() > 0) {
			...
			for (auto it = joinedGroupList.begin(); it != joinedGroupList.end(); it++) {
				cout << it->first << " " << it->second << endl;
				joinedIDGroupList.push_back(it->first);
			}
		...
		if (group == groupIdmsg && usernameLogin == response[2]) {
			cout << "la nhir" << endl;
			//showListMessageGroupChat();
			//listMessageGroup[group].push_back(msg);
			//cout << msg << endl;
			//showListMessageGroupChat();
			navigateScreen(CHAT_GROUP_BOX_SCREEN);
		}
		else {
			if (group == groupIdmsg && curScreen == LIST_MESSAGE_SCREEN && usernameLogin != response[2]) {
				cout << msg << endl;
			}
		}
...
void navigateScreen(SCREEN screen) {
	...
	case CHAT_GROUP_SCREEN:
		...
		handleChatGroupMenu();
		break;
	case CHAT_GROUP_BOX_SCREEN:
		showGroupChatBox(joinedGroupList.at(group).c_str());
		handleGroupChatBox();
		break;
